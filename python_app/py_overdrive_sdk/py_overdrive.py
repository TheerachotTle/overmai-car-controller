...
PIECES = {33:1, 17:2, 18:3, 36:4, 23:5, 20:6, 34:7}

class Overdrive:

    def __init__(self, host, port, uuid, driving_policy=None, verbose=False,sio=None):
        """
    ...
    def _location_changed_thread(self):
        while self._connected:
            try:
                (location, piece, offset, speed, order, clockwise, notification_time) = self._queues['locations'].get_nowait()
                location_event = self.build_location_event(location, piece, offset, speed, order, clockwise, notification_time)
    ...
    def _handle_notification(self, command_id, data):
        ...
        if command_id == '0x27':
            ...
            order = PIECES[piece]
            self._queues['locations'].put((location, piece, offset, speed, order, clockwise, datetime.utcnow()))
        # transition notification
        elif command_id == '0x29':
            # piece, piecePrev, offset, direction = struct.unpack_from("<BBfB", data, 2)
            # self._queues['transition'].put((piece, piecePrev, offset, direction))
    ...
    def _standard_driving_policy(self, **kwargs):
        ...
        """
        # print(kwargs)
        if (kwargs['piece'] == 36 and (kwargs['location']%3 == 2)) or kwargs['location'] == 33:
    ...
    """
        UTILS FUNCTIONS
    """
    # need to add lane arg
    def build_location_event(self, location, piece, offset, speed, order, clockwise, notification_time):
        ...
        event_data['car_color']=self.car_color
        return event_data
